#!/bin/bash
# PreToolUse hook: Bash 명령에서 서버/데몬 직접 실행을 차단하고 사용자에게 직접 실행 요청
...
if echo "$COMMAND" | grep -qE '(pnpm|npm|yarn|node|python|uvicorn|gunicorn|flask|django|next|vite|tsx)\s.*&\s*$'; then
  echo "Blocked: 백그라운드 서버 실행 금지. 사용자에게 서버 실행 요청" >&2
  exit 2
fi

# nohup, disown 차단
if echo "$COMMAND" | grep -qE '\b(nohup|disown)\b'; then
  echo "Blocked: nohup/disown 금지. 사용자에게 요청" >&2
  exit 2
fi
...
if echo "$COMMAND" | grep -qE '\b(pnpm|npm|yarn|bun)\s+(dev|start|serve)\b'; then
  echo "Blocked: 서버 직접 실행 금지. 사용자에게 서버 실행 요청" >&2
  exit 2
fi
...
if echo "$COMMAND" | grep -qE 'lsof\s+-i.*\|.*kill'; then
  echo "Blocked: 포트 점유 프로세스 강제 종료 금지. 사용자에게 프로세스 정리 요청" >&2
  exit 2
fi
