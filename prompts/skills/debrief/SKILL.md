---
name: debrief
description: 코드 작업 완료 후 변경 내러티브 문서 생성. 변경 요약, 설계 판단 근거, 학습 포인트를 Logseq에 기록. Use when completing code work, after PR creation, reviewing what was done, or wanting to document changes for learning. /debrief, 작업 정리, 변경 기록, 회고.
user-invocable: true
...
코드 작업 완료 후 **왜 이렇게 했는지**, **전체 그림**, **학습 포인트**를 구조화하여 Logseq에 기록한다.
...
수집한 데이터를 기반으로 Logseq 페이지 내용을 구성한다.
...
- **Related**: 관련 Logseq 페이지를 `qmd search`로 찾아 `[[링크]]`로 연결
- pr 프로퍼티: PR이 없으면 생략
...
3. **연결 우선**: 고립된 문서가 아닌 Logseq 그래프의 일부로 — 관련 페이지와 반드시 `[[링크]]` 연결
