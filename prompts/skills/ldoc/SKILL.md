...
**Slug 생성 원칙**: Self-Descriptive (파일명만 봐도 작업 파악 가능)

상세 규칙: `resources/07-todo-naming-guide.md` 참조
...
요구사항 문서 작성, 상태 변경, 검색/분석을 위한 워크플로우입니다.

- **WF12**: 요구사항 작성 (EARS 패턴 기반)
- **WF13**: 상태 변경 (draft → proposed → approved → implemented)
- **WF14**: 검색 및 분석 (시스템별, 상태별, 카테고리별)

**상세 절차**: `resources/08-requirements-workflows.md` 참조
**EARS 패턴**: `resources/ears-guide.md` 참조
...
`ldoc` CLI로 TODO 목록 조회, 아카이브, 건강도 체크를 수행합니다.

**상세 사용법**: `resources/09-cli-usage.md` 참조
