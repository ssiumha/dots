...
| 2. SKILL.md | 트리거 시 | 핵심 워크플로우 (또는 INSTRUCTIONS.md 참조) | <5K (분할 시 <200) |
| 3. INSTRUCTIONS.md | 필요 시 | 상세 절차 (분할된 경우) | 원본 크기 |
| 4. 리소스 | 필요 시 | resources/, templates/ | 필요량 |

**설계 원칙**:
- SKILL.md는 **절차적 지식만** (무엇을, 언제, 어떻게)
- 상세 정보는 리소스로 분리 → 필요 시 Read
- SKILL.md 400줄 초과 시 INSTRUCTIONS.md 분할 검토 → `skill-optimizer` 활용
...
2. **Skill 카테고리 및 유형 선택**
   ...
   **카테고리** (Anthropic 공식 분류):
   ...
   | **Workflow 자동화** | 다단계 프로세스를 정형화하여 실행 | tdd, git-worktree, plan-creator |
...
5. **추가 파일 생성** (선택)
   ...
   **INSTRUCTIONS.md** (SKILL.md 400줄 초과 시):
   - SKILL.md body를 INSTRUCTIONS.md로 분리
   - SKILL.md에는 frontmatter + 참조문만 유지
   - 분할 작업은 `skill-optimizer` 활용
...
10. **Phase로 구조화**: 스킬을 Phase(단계)로 나누면 Claude가 건너뛰지 않고 순서대로 실행한다. Phase 없이 "알아서 해"는 일관성이 없다
11. **Scoring으로 판정 강제**: PASS/WARN/FAIL 같은 명시적 판정 기준이 있으면 Claude가 능동적으로 평가한다. 스코어링 없으면 "문제없어 보입니다"로 넘어간다. 설계 원칙 기반 스코어링은 `/principles` 참조

## 안티패턴

| ❌ 문제 | ✅ 해결 |
|--------|--------|
| SKILL.md 500줄 초과 | resources/로 상세 내용 분리, 400줄 초과 시 INSTRUCTIONS.md 분할 (`skill-optimizer`) |
| 프로젝트 특정 정보 포함 | placeholder 사용 ({project}, {name}) |
| 다른 skill과 내용 중복 | 참조로 대체 ("tdd 참조") |
