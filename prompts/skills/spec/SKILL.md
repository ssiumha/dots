---
name: spec
description: "스펙 작성 가이드. 6 core areas + EARS 요구사항 패턴 + Always/Ask/Never 경계 시스템. Use when writing SPEC.md, defining requirements, 요구사항 정의, or 스펙 작성."
---

# Spec

Addy Osmani의 "Good Spec" 원칙 + EARS 요구사항 패턴 기반의 프로젝트 스펙 작성 가이드입니다.
...
요구사항 문장의 모호성을 제거하는 5가지 문장 패턴. 각 요구사항을 작성할 때 아래 패턴 중 하나를 선택한다.

| 패턴 | 형식 | 언제 |
|------|------|------|
| **Ubiquitous** | "The system shall [action]" | 항상 참인 요구사항 |
| **Event-Driven** | "**When** [event], the system shall [action]" | 특정 이벤트 발생 시 |
| **State-Driven** | "**While** [state], the system shall [action]" | 특정 상태에서 |
| **Unwanted** | "**If** [condition], **then** the system shall [action]" | 예외/에러 처리 |
| **Optional** | "**Where** [feature], the system shall [action]" | 옵션 기능 |

예시:
- Ubiquitous: "시스템은 모든 API 요청에 대해 응답 시간을 로깅한다"
- Event-Driven: "사용자가 로그인하면, 시스템은 마지막 접속 시간을 갱신한다"
- State-Driven: "결제 처리 중에, 시스템은 중복 요청을 차단한다"
- Unwanted: "DB 연결이 실패하면, 시스템은 3회 재시도 후 관리자에게 알림을 보낸다"
- Optional: "알림 기능이 활성화된 경우, 시스템은 이메일을 발송한다"

> **WHY EARS**: "시스템은 적절히 처리한다" 같은 모호한 문장 대신, 조건과 행동을 명시적으로 분리한다.

## SPEC Lifecycle

스펙 문서의 수명을 의식적으로 관리한다:

| 레벨 | 의미 | 적합한 상황 |
|------|------|-----------|
| **일회성** (spec-first) | 구현 후 폐기. SPEC → 코드로 녹아듦 | 단순 기능, 스파이크 |
| **유지형** (spec-anchored) | 구현과 함께 업데이트 | 대부분의 프로젝트 |
| **원본형** (spec-as-source) | 스펙이 진실의 원천 | API 계약, 규격 문서 |

Phase 1에서 프로젝트의 SPEC Lifecycle 레벨을 결정하고, Phase 4에서 그에 맞게 관리한다.
