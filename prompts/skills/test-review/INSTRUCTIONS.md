...
4. **test-guidelines 참조**: 품질 기준은 test-guidelines의 체크리스트를 기반으로 하되, 여기서 중복 기술하지 않는다
...
> 기준: test-guidelines의 품질 체크리스트 참조
...
| **WARN** | "아래 항목을 확인하세요: [WARN 목록]. 테스트 작성 기준은 `/test-guidelines`를 참조하세요." |
| **FAIL** | "아래 항목을 수정해야 합니다: [FAIL 목록]. `/test-guidelines`로 작성 기준을 확인한 뒤 수정하세요." |

### 관련 Skill 제안

| 상황 | 제안 |
|------|------|
| 테스트 작성/수정 필요 | `/test-guidelines` |
| TDD 워크플로우 필요 | `/tdd` |
| 보안 관련 테스트 부족 | `/review-security` |
| Mock 구조 문제 | `/test-guidelines` (Workflow 2: Mock 과다 진단) |
...
**Verdict**: FAIL — High 이슈 1건 수정 필수. `/test-guidelines`로 작성 기준을 확인하세요.
```
...
```
test-guidelines (테스트 작성)
  → test-review (테스트 리뷰)  ← 현재 skill
    → test-guidelines (수정 가이드)
...
| Skill | 관계 | 설명 |
|-------|------|------|
| `test-guidelines` | 보완 | 작성 기준 참조, 수정 시 안내 |
| `tdd` | 선행 | TDD 완료 후 품질 검증으로 연결 |
| `review-security` | 위임 | 보안 테스트 심층 검토 |
