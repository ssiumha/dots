...
- **Tidying ≠ Refactoring**: 몇 분 내 완료되는 작은 정리 — "Tidying is geek self-care"
- **구조 변경(S) vs 행동 변경(B) 분리**: 하나의 변경에 구조와 행동을 섞지 않는다
- **1 tidying = 1 commit**: 각 tidying은 반드시 별도 커밋
- **연속 실행 가능**: 여러 tidying 수행 시 여러 커밋 생성
- **가역성 원칙**: 대부분의 SW 설계 결정은 쉽게 되돌릴 수 있다 — 가역적 결정은 빠르게 내린다
...
| 3 | Explaining Variable | 복잡한 조건문 | 의도 명확화 |
| 4 | Explaining Constant | 매직 넘버/문자열 | 의미 부여 |
| 5 | Normalize Symmetries | 비일관적 패턴 | 일관성 확보 (⚠️ 우연한 유사성 주의) |
| 6 | Chunk Statements | 빈 줄 없는 긴 문장 나열 | 논리 단위 시각화 |
| 7 | Explicit Parameters | 전역/컨텍스트 암묵 의존 | 의존성 명시화 |
| 8 | Extract Helper | 3회+ 반복 코드 | 중복 제거 |
| 9 | Reading Order | 함수 순서 불일치 | 흐름 개선 |
| 10 | Cohesion Order | 관련 코드 분산 | 응집도 향상 |
...
### 판단 가이드: 언제 정리할 것인가 (Part II)

Survey 후 적용 전, 타이밍을 판단한다:

| 타이밍 | 조건 | 행동 |
|--------|------|------|
| **First** | 다음 변경이 정리하면 쉬워질 때 | 지금 정리 후 변경 |
| **After** | 변경 완료 후 구조 개선 기회 발견 | 변경 커밋 후 별도 정리 |
| **Later** | 관련 없지만 발견한 문제 | 목록에 기록, 나중에 처리 |
| **Never** | 비용 > 가치, 또는 코드가 안정적 | 정리하지 않음 |

**리듬**: 1시간 이상 정리에 빠지면 → 더 작은 최소 변경을 놓쳤을 가능성. 멈추고 재평가.
**배치 크기**: 당장 필요한 범위만 정리. 파레토 분포 — 소수의 정리가 대부분의 가치를 제공.

상세: `resources/02-managing-tidyings.md`
...
1. **cost(SW) ≈ coupling** (Constantine's Equivalence): 소프트웨어 비용의 대부분은 결합도에서 온다. 정리의 궁극적 목표는 불필요한 결합 감소. 상세: `resources/03-theory.md`
2. **동작 변경 금지**: tidying은 구조만 변경, 동작은 그대로
3. **테스트 통과 유지**: 기준 커밋(tidy 제외 최근 커밋)과 동일한 테스트 결과 유지
...
4. **1 tidying = 1 commit**: 여러 tidying 실행 시 각각 별도 커밋
5. **되돌리기 쉽게**: 독립적인 커밋으로 개별 revert 가능
...
- `resources/02-managing-tidyings.md`: Part II — 정리 시점과 관리
- `resources/03-theory.md`: Part III — 구조, 결합, 경제학
